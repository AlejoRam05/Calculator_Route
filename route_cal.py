...
def obtener_posicion(mensaje, FILAS, COLUMNAS):
    while True:
        try:
...
OBJETIVO = obtener_posicion("el objetivo", FILAS, COLUMNAS)
while STAR == OBJETIVO:
    print("El objetivo no puede estar en la posición de inicio (0,0).")
    OBJETIVO = obtener_posicion("el objetivo", FILAS, COLUMNAS)

def obtener_obstaculos(FILAS, COLUMNAS, matriz):
    obstaculos = []
    while True:
        try:
...
OBSTACULOS = obtener_obstaculos(FILAS, COLUMNAS, matriz)
